document.addEventListener("DOMContentLoaded", () => {
  ...
  const adminEventsTableBody = document.getElementById("adminEventsTableBody");
  ...
  const renderEvents = () => {
    if (!adminEventsTableBody) return;
    ...
    adminEventsTableBody.innerHTML = events
      .map(
        (event) => `
          <tr>
    ...
    const radioButtons = adminEventsTableBody.querySelectorAll(".event-radio");
    radioButtons.forEach((radio) => {
      radio.addEventListener("change", (e) => {
        if (e.target.checked) {
          selectedEventId = Number(e.target.value);
          // Agregar clase a la fila seleccionada para el estilo
          const rows = adminEventsTableBody.querySelectorAll("tr");
  ...
  const handleRegisterAttendance = async () => {
    ...
    const formValues = {
      numero_identificacion: (
        document.getElementById("attendeeDocumentModal")?.value || ""
      ).trim(),
      nombres: (
        document.getElementById("attendeeFirstNameModal")?.value || ""
      ).trim(),
      apellidos: (
        document.getElementById("attendeeLastNameModal")?.value || ""
      ).trim(),
      correo_electronico: (
        document.getElementById("attendeeEmailModal")?.value || ""
      ).trim(),
      numero_celular: (
        document.getElementById("attendeePhoneModal")?.value || ""
      ).trim(),
      cargo: (document.getElementById("attendeeRoleModal")?.value || "").trim(),
      empresa: (
        document.getElementById("attendeeCompanyModal")?.value || ""
      ).trim(),
  ...
  const attendeesCardsContainer = document.getElementById(
    "attendeesCardsContainer"
  );
  ...
  const renderAttendees = () => {
    ...
    if (!filtered.length) {
      if (attendeesTableBody) {
        attendeesTableBody.innerHTML =
          '<tr><td colspan="10">No se encontraron asistentes</td></tr>';
      }
      if (attendeesCardsContainer) {
        const cardsGrid = attendeesCardsContainer.querySelector(
          ".attendees-cards-grid"
        );
        if (cardsGrid) {
          cardsGrid.innerHTML =
            '<div class="attendee-card-empty">No se encontraron asistentes</div>';
        }
      }
      return;
    }
    ...
    if (attendeesTableBody) {
      attendeesTableBody.innerHTML = filtered
        .map((assist) => {
          ...
          const statusColor =
            Number(assist.asiste) === 1 ? "#22c55e" : "#ef4444";
